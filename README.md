...
Single Odoo addon: `oca_design_system`.

## What's inside

- **Tokens** — `static/src/scss/_tokens.scss` declares one accent
  variable (`--oca-accent`), eight bucket colors, a per-extension
  palette, and the Sass mixins that emit data-attribute-keyed
  selectors. Every fallback maps to an Odoo 19 `$o-*` token.
- **OWL components**
  - `<OcaChip variant="ext|size|count|warning|success" label="…"/>`
  - `<OcaInitialsAvatar name="…"/>`
  - `<OcaCardTile accent="ext|initial|preview" …/>`
...
generalized into the `--oca-*` variables and `<OcaChip>` component here.
...
cd custom/src
git clone git@github.com:ledoent/oca-design-system.git
```

Then add `oca_design_system` to `addons.yaml` and depend on it from
