...
const roadmap = JSON.parse(
    readFileSync(resolve(__dirname, "..", "roadmap.json"), "utf8"),
);
...
async function main() {
    ...
    console.log(`  Pages:   ${surfaces.pages.length} + 1 (Roadmap)`);
    ...
    // 3. One page per surface — Penpot creates a default empty Page-1;
    //    rename it to the first surface, then add the rest.
    for (let i = 0; i < surfaces.pages.length; i++) {
        const page = surfaces.pages[i];
        const action = i === 0 ? "rename-page" : "create-page";
        await rpc(action, {
            "file-id": file.id,
            ...(i === 0 ? {id: file["page-id"]} : {}),
            name: page.name,
    ...
    // 4. Roadmap page — one rectangle + text card per item.
